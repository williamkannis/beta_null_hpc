#-------------------------------------------------------------------------------
#
#  Phylogenetic beta diversity null models cluster code
#
#-------------------------------------------------------------------------------

# Author:

# Created: 02/14/2026

# Description: Batch estimates phylogenetic beta diversity and LCBD for a list of
# null model assemblages and exports files to directory. For use on the Palmetto
# cluster.

# House keeping  ---------------------------------------------------------------
rm(list = ls())

# Packages
library(dplyr)
library(purrr)
library(tibble)
library(BAT)
library(adespatial)
library(ade4)
library(parallel)

# Directories
out_dir <- "null_out"

# Load costume functions
source("diversity_batch_functions.R")

# Input data
cores <- 37
args <- commandArgs(trailingOnly = TRUE)
input_list <- readRDS(args)
com <- input_list$com
tree_list <- input_list$phy_list

# Output naming
time <- input_list$time
iter <- names(tree_list)
min_iter <- iter[as.numeric(iter) == min(as.numeric(iter))]
max_iter <- iter[as.numeric(iter) == max(as.numeric(iter))]


# Null model batch processing  -------------------------------------------------

# Run beta function
out_list <- mclapply(tree_list,phy_beta_batch,com = com, mc.cores = cores)

# Check for scheduled core issue, do not export results if it exists
export_results <-all(sapply(out_list, function (x) length(x) == 2))

# Export outputs
if(export_results) {
  out_name <- paste0(time,"_phy_beta_null_",min_iter,"-",max_iter,".rds")
  saveRDS(out_list,file.path(out_dir,out_name))
} else {
  print("some scheduled cores did return values. all jobs affected")
}


# End of script

