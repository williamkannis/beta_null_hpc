...
final_list <-lapply(analysis_list, function (x) {
  ...
  # colnames(ls$ses) <- paste0(colnames(ls$ses),"_ses")  # use this code if using ses rather than es
...
lapply(plot_metrics,function(i){
  ...
  # Plot
  obs_box <- ggplot(obs_plot_df, aes(x = name, y = value, fill = name,color = name)) +
    geom_boxplot()+
...
lapply(plot_metrics,function(i){
  ...
  # Plot
  es_box <- ggplot(es_plot_df, aes(x = name, y = value, fill = name,color = name)) +
    geom_boxplot()+
    stat_summary(geom = "crossbar", 
                 width = 0.75,         
                 fatten = 2,         
...
# Obs correlation
d_lcbd %>% select(tax_Btotal,tax_Brepl,tax_Brich,
                  fun_Btotal,fun_Brepl,fun_Brich,
                  phy_Btotal,phy_Brepl,phy_Brich) %>% cor()

# ES correlation
d_lcbd %>% select(tax_Btotal,tax_Brepl,tax_Brich,
                  fun_Btotal_es,fun_Brepl_es,fun_Brich_es,
                  phy_Btotal_es,phy_Brepl_es,phy_Brich_es) %>% cor() 
