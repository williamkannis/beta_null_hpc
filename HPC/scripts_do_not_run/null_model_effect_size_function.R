...
ses_fun <- function(obs,null_list,emp_padding=0,diag = T, mc.cores = 1) {
  ...
  # Format all input data into vectors (obs) and matrices (null) for 
  ...
  if (is.matrix(obs) | is.data.frame(obs)) {
    stopifnot("please supply null and obs values in same format" = 
                all(dim(obs) == dim(null_list[[1]])))
    stopifnot(
      "If matrix, input has to be less than 3 dimensions"=length(dim(obs)) < 3
      )
    obs <- as.matrix(obs)
    ...
    null <- do.call(
      ...
      lapply(
        null_list,
        function(x) unlist(as.vector(as.matrix(x)))
        )
      )
  }
  ...
  emp_p <- sapply(1:length(obs_format), function(i){
    ...
    p[p==1] <- 1-1/length(n)  # if p value is 1, apply the padding to 
    # upperside to prevent -inf es
    p
  })
  
  ## Estimate effect size based on empirical p  ##
